...
const proxy = 'http://212.132.120.102:11584/proxy?url=';
...
async function transcribeAudio(filePath, apiKey) {
    ...
    // Gunakan salah satu key cadangan jika ada, fallback ke string kosong jika groq tidak dipakai langsung
...
async function executeTool({ action, payload, m, sock, quoted, bufferMedia, mimetype, groqApiKey }) {
    switch (action) {
        ...
        case 'EDIT_IMAGE':
            ...
            const edited = await editImage(bufferMedia.toString('base64'), payload);
...
- EDIT_IMAGE : edit gambar yang direply (fallback) |prompt
...
// Formatting Markdown AI ditangani oleh helper terpusat:
// Func.formatMarkdown / func.formatMarkdown (lib/Function/markdown.js).
