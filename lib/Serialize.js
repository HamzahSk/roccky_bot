...
import { downloadContentFromMessage, getContentType, normalizeMessageContent } from 'baileys'
import fs from 'fs'
...
const cleanMessageType = (messageType) =>
   messageType.slice(-7) === 'Message' ?
      messageType.slice(0, -7) :
      messageType

function download() {
   return downloadAsBuffer(this.msg, cleanMessageType(this.type))
}

function downloadQuoted() {
   return downloadAsBuffer(this, cleanMessageType(this.type))
}
...
m.reply = async (text, options = {}, extra = {}) => {
   const replyStyle = setting.replyStyle
   let quote = m // Default quote adalah pesan saat ini

   // --- Style 2: Request Payment Style
   if (replyStyle == 2) {
      quote = {
         key: {
   ...
   // --- Style 4: Document/Troli "Ourin" Style (Gaya yang kamu minta)
   else if (replyStyle == 4) {
      const quotedMsg = {
         key: {
         ...
         message: {
            contactMessage: {
               displayName: `🪸 ${botName || 'Roccky Bot'}`,
      ...
      path.join(process.cwd(), 'package.json')),
         mimetype: "image/png",
         footer,
         fileName: botName || 'Roccky',
         fileLength: 99999999999999,
         jpegThumbnail: await Func.resizeImage(botThumbnail, 252),
         caption: text,
         ...options
      }, { quoted: quotedMsg, ...extra })
   }
   // --- Style 5: Premium Ad Style (Menggunakan fitur externalAdReply fork baileys)
   else if (replyStyle == 5) {
      return sock.sendMessage(m.chat, {
         text: text,
         externalAdReply: {
            title: `✨ ${botName || 'Roccky System'}`,
            body: `User: ${m.pushName}`,
            thumbnailUrl: botThumbnail, // Contoh URL
            largeThumbnail: true,
            mediaType: 1,
            renderLargerThumbnail: true,
            sourceUrl: 'https://github.com/itsliaaa/Roccky'
         },
         ...options
      }, { quoted: m, ...extra })
   }
   // --- Style 6: Newsletter Style (Tampilan seolah pesan diteruskan dari Newsletter)
   else if (replyStyle == 6) {
      return sock.sendMessage(m.chat, {
         text: text,
         contextInfo: {
            forwardingScore: 999,
            ...
            forwardedNewsletterMessageInfo: {
               newsletterJid: saluranJid,
               serverMessageId: 101,
               newsletterName: `${botName} · Official Updates`
            }
         },
         ...options
      }, { quoted: m, ...extra })
   ...
   // Default fallback (menggunakan style 1 atau default sendText)
